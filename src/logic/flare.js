...
export function buildFlareEmbed({ displayName, monsterEmoji, typeEmoji, typeLabel, strengthRating, hostUsername, sessionId, members = [] }) {
  ...
  return new EmbedBuilder()
    .setTitle(`🚨 SOS Flare: ${displayName}!`)
    .setDescription([
  ...
  async execute(interaction) {
    ...
    if (crownsRes.rows.length === 0) {
      return interaction.reply({
        content: `You don't have any crowns for **${monster.name}**. Use \`/crown add\` first!`,
    ...
    if (!sessionId) {
      return interaction.reply({
        content: "Set your Lobby ID with `/profile settings` first, or pass it as `session_id` option.",
        flags: MessageFlags.Ephemeral,
      });
    }
